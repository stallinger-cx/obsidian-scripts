...
/**
 * @classdesc Provides dashboard views for FinBase.
 */
class FinBaseDashboard {

    /**
     * Determines if the descriptor feature is enabled in the current DataView context.
     *
     * @param {DataviewInlineApiInstance} dv
     * @returns {boolean} - true if the descriptor feature is enabled
     */
    ...
    /**
     * Executes an anonymous async function within the context of the FinBase DataViewServiceContainer.
     *
     * @async
     * @param {DataviewInlineApiInstance} dv
     * @param {Function} asyncFunction - anonymous async function to be executed within the context
     */
    ...
    /**
     * Displays a test-table with spcecific format settings.
     *
     * @async
     * @param {DataviewInlineApiInstance} dv
     */
    async displayTestTable(dv) {
        await this._run(dv, async(
            /** @type {{ renderer: ViewRendererInstance; formatter: ViewFormatter; }} */ container
        ) => {
            const { renderer, formatter } = container;

            let data = [
                { per: "amount", date: moment(), amount: 23434.34545, test: dv.current().file.link, tags: '' },
                { per: "whole amount", date: new Date(), amount: 4, test: dv.current().notes, tags: '' },
                { per: "link-array", date: '2025', amount: 12, test: [dv.current().file.link, dv.current().file.link], tags: '' },
                { per: "relating files", date: 2026, amount: -501, test: dv.current().relates_to, tags: '' },
                { per: "tags in autoformat col", date: '2025-12-12', amount: 0, test: dv.current().tags, tags: 'xx' },
                { per: "special | chars", date: null, amount: -78108.8999, test: "a simple |", tags: dv.current().tags?.sort() }
            ];
            ...
            const tableConfig = [
                ...
                {
                    key: 'test',
                    minWidth: "400px", // !! table could get scroll-bars!
                    align: (v) => formatter.alignLeft(v),
                    format: (v) => formatter.default(v)
                },
                {
                    key: 'tags',
                    heading: '🏷️ *tags*',
                    format: (val) => formatter.hashtag(val)
                }
            ];
            renderer.renderTable(data, tableConfig, '70%');
        });
    }
    ...
    /**
     * High-Level: Lädt verknüpfte Notizen und extrahiert spezifische Tags.
     * @param {Object} row - Die aktuelle Zeile (Dataview Page).
     * @param {string} linkKey - Der Key für die Verknüpfung (z.B. "relates_to").
     * @param {string} tagPrefix - Der gesuchte Tag-Start (z.B. "finance/asset/").
     * @returns {string[]} - Bereinigte Liste relevanter Tags.
     */
    _getRelatedTags(row, linkKey, tagPrefix) {
        // ToDo: besseren Ort finden!!!
        // FIXME
        // 1. Links holen (Safeguard)
        const links = row[linkKey];
        if (!links) return [];

        // 2. Pages laden (Methode 1 nutzen)
        const pages = this.resolvePages(links);

        // 3. Tags sammeln und filtern
        const foundTags = new Set(); // Set gegen Duplikate

        for (const page of pages) {
            // Checken ob Tags da sind
            if (page.file && page.file.tags) {
                // Filtern (Methode 2 nutzen)
                const relevantTags = this._filterTagsByPrefix(page.file.tags, tagPrefix);

                // Zum Set hinzufügen
                relevantTags.forEach(t => foundTags.add(t));
            }
        }
    ...
    /**
     * Displays the savings plans view.
     *
     * @async
     * @param {DataviewInlineApiInstance} dv
     * @param {boolean} [filterRelated=true] - If true, only savings plans related to the current file are displayed.
     */
    async displaySavingsPlans(dv, filterRelated = true) {
        ...
        await this._run(dv, async(
            /** @type {{ dataLoader: DataLoaderInstance; dataSorter: DataSorterInstance; dataFilter: DataFilterInstance; dataAggregator: DataAggregatorInstance; renderer: ViewRendererInstance; formatter: ViewFormatter; }} */ container
        ) => {
            const c = customJS.Constants;
            const fbc = customJS.FinBaseConstants;
            const relatesToFile = dv.current().file.link;
            const { dataLoader, dataSorter, dataFilter, dataAggregator, renderer, formatter } = container;
            const filterContext = customJS.DataParams.createFilterContext({
                    folders: c.FOLDER.NOTES,
                    tags: fbc.TAG.SAVINGS_PLAN,
                    filters: filterRelated ? { [c.PROPERTY.RELATES_TO]: dv.current().file.link } : {}
            });
            // load savings plan entries, relating to current file
            let entries = dataLoader.getNotesFiltered(filterContext);
            // map table values
            entries = entries
                .map(p => {
            ...
            let sums = [
                { per: "month", amount: sumPerMonth, test: dv.current().file.link, hugo: '' },
                { per: "week", amount: (sumPerMonth / 4), test: dv.current().notes, hugo: '' }
            ];
            // maps the object above via 'key' (could also be a dataview-array!)
            const tableConfig = [
                {
                    key: "amount",
                    heading: "*active* 💰 amount",
                    format: (/** @type {number} */ val) => formatter.currency(val)
                },
                {
                    key: 'per'
                },
            ];
            renderer.renderTable(sums, tableConfig, '50%');

            if (!relatesToFile) {
                // ToDo: load asset-tag per savings-plan, group and sum by it
                // see new methods in: DataFilter, DataLoader and here!
            }
        });
    }

    /**
     * Displays the current worth of an asset by loading data from 'worth' log entries.
     *
     * @async
     * @param {DataviewInlineApiInstance} dv
     */
    async displayWorth(dv) {
        await this._run(dv, async(
            /** @type {{ dataLoader: DataLoaderInstance; dataAggregator: DataAggregatorInstance; }} */
            container
        ) => {
            const c = customJS.Constants;
            const relatesToFile = dv.current().file.link;
            ...
            let entries = dataLoader.getNotesFilteredCompat(
                    c.FOLDER.LOGS,
                    [],
