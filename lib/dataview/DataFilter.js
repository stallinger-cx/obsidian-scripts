    ...
    static Instance = class {
        ...
        filterCostRelatedEntries(entries) {
            ...
            this._descriptor.push(
                `Filtering entries with \`${this._consts.PROPERTY_COST}\` or \`${this._consts.PROPERTY_COST_INCOME}\` defined and where \`${this._consts.PROPERTY_COST_PER}\` is not set.`, 1);
            return entries
                .filter(p =>
                    // cost properties exist
                    !((p[this._consts.PROPERTY_COST] === undefined)
                        && (p[this._consts.PROPERTY_COST_INCOME] === undefined))
                    // cost only, where no 'cost_per' is set
                    && !(typeof p[this._consts.PROPERTY_COST_PER] === 'number')
                );
        }
    }
