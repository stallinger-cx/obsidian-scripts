...
class DataLoader {
    ...
    static Instance = class {
        ...
        /**
         * Get real pages from links or paths.
         *
         * @param {DataElementLink|DataElementLink[]|string|string[]} links - Link or array of links/paths
         * @returns {Record<string, any>[]} - data pages (array of entry-objects)
         */
        resolvePages(links) {
            // todo: test!
            if (!links) return [];
            const linkList = [].concat(links);
            return linkList
                .map(l => {
                    const path = l.path ? l.path : l; // todo: path detaction to central method?!
                    return path ? this._dv.page(path) : null;
                })
                .filter(p => p !== null); // remove invalid pages
        }
    }
